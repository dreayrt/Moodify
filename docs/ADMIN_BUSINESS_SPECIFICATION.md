...
## 3. ĐẶC TẢ CHI TIẾT CÁC KHỐI NGHIỆP VỤ QUẢN TRỊ ADMIN (STUDIO COMMAND COCKPIT)

> [!IMPORTANT]
> **Phân Tách Trách Nhiệm Kiến Trúc (Separation of Concerns)**:
> - **Admin Console (`/dashboard/admin`)**: Tập trung 100% vào vai trò Quản trị viên Tối cao — Thống kê BI điều hành, Quản trị người dùng & IAM, Quản lý kho nhạc MongoDB & Vibe cảm xúc, Cấu hình gói cước/dòng tiền và Hộp đen kiểm toán (Audit Vault).
> - **Moderator Dashboard (`/dashboard/moderator`)**: Tách riêng cho đội ngũ Chuyên viên Kiểm duyệt — Thẩm định bài hát/album nghệ sĩ tải lên, duyệt ca từ, kiểm tra chất lượng âm thanh.
> - **Distributor Portal (`/dashboard/distributor`)**: Tách riêng cho các Đối tác Nhà phân phối bản quyền — Quản lý hợp đồng phân phối, tỷ lệ chia sẻ doanh thu (`revenue_share`), chứng thư bản quyền số.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                   MOODIFY ADMIN STUDIO COMMAND COCKPIT                      │
├─────────────────────────────────────────────────────────────────────────────┤
│ 1. Executive BI & Telemetry│ Thống kê tổng hợp, Live MySQL & MongoDB status │
│ 2. IAM & User Governance   │ Quản lý 189 tài khoản, phân quyền, khóa/mở     │
│ 3. Music Catalog & Audio AI│ Quản lý 147 bài hát, Audio Radar, Vibe AI, Takedown│
│ 4. Monetization & Billing  │ Bảng giá dịch vụ động, đối soát giao dịch VNPAY│
│ 5. System Settings & Vault │ Cấu hình AI threshold, giới hạn DRM, Audit log │
└─────────────────────────────────────────────────────────────────────────────┘
```
